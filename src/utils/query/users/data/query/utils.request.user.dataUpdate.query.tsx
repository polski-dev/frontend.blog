...
export async function userDataAvatarUpdateFrontEnd({ file, authToken }: { file: File; authToken: string }): Promise<UserDataAvatarUpdateType> {
  const count: AxiosResponse<UserDataAvatarUpdateType> = await axios.put(`https://polskidev.herokuapp.com/api/user/data/avatar`, { avatar: file }, { headers: { Authorization: `Bearer ${authToken}`, "Content-Type": "multipart/form-data" } });
  return count.data;
}
