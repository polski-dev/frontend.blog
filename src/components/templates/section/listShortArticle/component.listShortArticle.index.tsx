...
export default function SectionShortArticle({ data, type }: any) {
  ...
  useEffect(() => {
    ...
    function loadArticle() {
      ...
      check = setTimeout(() => {
        ...
        else if (type === "articleWaitingRoom" && story.content.article.waitingRoom.meta.pagination.page < story.content.article.waitingRoom.meta.pagination.pageCount && !iAmWaitingForAnswer && heightEl - height < 0)
        ...
        console.log(story.search.all.meta.pagination.page);
        console.log(story.search.all.meta.pagination.pageCount);
      }, 500);
    }
  ...
  useEffect(() => {
    ...
    else if (type === "search" && !!dataAPI && iAmWaitingForAnswer) dispatch(addSearchQuery(dataAPI));
    if (type === "search" || type === "article" || type === "articleWaitingRoom" || type === "video" || type === "videoWaitingRoom" || (type === "all" && !!dataAPI && iAmWaitingForAnswer)) setIamWaitingForAnswer(false);
  ...
  return (
    <Section>
      ...
      <Options></Options>
      {data.map((item: any, i: number) => selectTemplateForContent(item, i))}
