...
import { Section, Title, Options, Article, BoxContent, BoxAuthor, BoxAuthorImg, AuthorData, AuthorName, DateAdded, TitleArticle, ListTags, Tag, ListStats, Item, BoxInformation, Info } from "./content.shortArticle.style";
...
const SectionShortArticle = ({ data, type, title }: any) => {
  ...
  useEffect(() => {
    if (readData.checkIsAnyNextContent(story) === true) !isAnyNextContent && setIsAnyNextContent(true);
    else isAnyNextContent && setIsAnyNextContent(false);
  }, [story, type, isAnyNextContent, readData]);
  ...
  useEffect(() => {
    if (iAmWaitingForAnswer) {
      fetch(`${slug.setContentApi}${story.article.best.pageActive + 1}`)
        .then((r) => r.json())
        .then((d) => {
          if (!!d.data.length) dispatch(addArticleBest({ data: d.data }));
