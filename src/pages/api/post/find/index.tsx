...
export default async function postFindApi(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  if (req.method === "GET") {
    const { typ, page, published, id, tagId } = req.query;
    ...
    if (!(typeof typ === "string") || (typ != ContentEnum.post && typ != ContentEnum.article && typ != ContentEnum.video && typ != ContentEnum.userPost))
    ...
    else if (!!tagId && !parseInt(typeof tagId === "string" ? tagId : "0")) return res.status(400).json(MessageErrorInAPI({ name: "Wrong field tagId", message: "Wrong field page you can fix id tag , id tag must as number miniumum 1" }));
      ...
      return res.status(200).json(
        await postsFindBackEnd({
          ...
          tagId: parseInt(typeof tagId === "string" ? tagId : "0"),
        })
      );
